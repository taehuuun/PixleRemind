...
public class SplashUI : BaseSceneUI
{
    ...
    {
        LoadingManager.Instance.AddTask(GPGSHelper.Init, "구글 플레이 서비스 초기화...");
        LoadingManager.Instance.AddTask(LoginManager.Login, "로그인...");
        LoadingManager.Instance.AddTask(DataSyncManager.SyncUserDataAndLocalData, "유저 / 로컬 데이터 동기화 중...");
        LoadingManager.Instance.StartNextSceneLoading(SceneNames.MainScene);
        yield return null;
    }
}